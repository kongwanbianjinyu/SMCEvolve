...
def create_program(problem: Dict[str, Any]) -> str:
    ...
    problem_dir = os.path.join(
        ...
        "generated",
        problem["dataset_identifier"],
        str(problem["equation_idx"]),
    )
...
def create_evaluator(problem: Dict[str, Any]) -> str:
    ...
    # Save data files (absolute paths under the problem directory).
    x_train_path = os.path.join(problem_dir, "X_train_for_eval.npy")
    y_train_path = os.path.join(problem_dir, "y_train_for_eval.npy")
...
'''
    ...
    # Write the upstream OpenEvolve evaluator (verbatim logic) under the
    # `openevolve_evaluator.py` filename so the SMCEvolve wrapper can
    # import from it unambiguously.
    evaluator_file_path = os.path.join(problem_dir, "openevolve_evaluator.py")
    with open(evaluator_file_path, "w") as f:
        f.write(evaluator_script_content)

    # Emit the SMCEvolve wrapper `evaluator.py` next to it.
...
'''
    ...
    # Emit a concise task.md prompt for the SMCEvolve LLM proposer.
    _input_names = ", ".join(symbols[i] for i in input_indices) or "none"
    _output_name = symbols[output_index]
    task_md = f"""# Symbolic Regression: {problem["dataset_identifier"]} / {problem["equation_idx"]}

Target output: **{_output_name}**.
Input features (columns of `x`, in order): **{_input_names}**.

Write a Python function

```python
def func(x: np.ndarray, params: np.ndarray) -> np.ndarray:
    '''x: (n_samples, {num_input_features})   params: ({model_num_params_expected},)   returns: (n_samples,)'''
```
...
"""
    with open(os.path.join(problem_dir, "task.md"), "w") as f:
        f.write(task_md)
...
def process_problem(initialized_dataset, problem_id: int, split_name: str) -> str:
    ...
    try:
        ...
        essential_files = [
            ...
            os.path.join(problem_dir, "openevolve_evaluator.py"),
            os.path.join(problem_dir, "evaluator.py"),
            os.path.join(problem_dir, "task.md"),
