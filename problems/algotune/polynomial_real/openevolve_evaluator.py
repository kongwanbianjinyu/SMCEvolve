"""
Evaluator for the polynomial_real task with baseline comparison
...
if setup_algotune_paths():
    try:
        ...
        # Import the specific polynomial_real task to register it
        from AlgoTuneTasks.polynomial_real.polynomial_real import PolynomialReal
        print("Successfully imported AlgoTune tasks and polynomial_real")
...
def evaluate(program_path, config=None):
    """
    Enhanced evaluation with baseline comparison for polynomial_real task.
    ...
    try:
        ...
        task_class = None
        if "polynomial_real" in TASK_REGISTRY:
            task_class = TASK_REGISTRY["polynomial_real"]
            print(f"Successfully loaded polynomial_real task from registry")
        else:
            print(f"Error: polynomial_real task not found in TASK_REGISTRY")
            print(f"Available tasks: {list(TASK_REGISTRY.keys())}")
            raise Exception("Could not load polynomial_real task from AlgoTune registry")
            ...
            try:
                ...
                if evolved_result["results"]:
                    ...
                    try:
                        ...
                        evolved_solver = program.PolynomialReal()
