...
class AlgoTuneTaskAdapter:
    ...
    def _extract_task_class_info(self, task_name: str) -> Dict[str, Any]:
        ...
        class_info = {
            ...
            'imports': [],
            'class_code': None,
            # Module-level helpers that live alongside the task class but
            # outside it (e.g. `def _is_empty(x): ...`,
            # `def _single_thread_blas(): ...`, plus any try/except that
            # defines a name those helpers depend on). Without these,
            # the extracted solve/is_solution method bodies raise NameError.
        ...
        # Collect module-level helpers (top-level only, not recursive):
        # functions defined outside any class and any try/except blocks
        # that set up names those helpers depend on. The AlgoTune task
        # class itself is handled separately below.
        if hasattr(ast, 'unparse'):
            for node in tree.body:
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    class_info['module_level_helpers'].append(ast.unparse(node))
                elif isinstance(node, ast.Try):
                    # e.g. `try: from threadpoolctl import threadpool_limits
...
'''
    ...
    def _generate_initial_program(self, task_name: str) -> str:
        ...
        # Module-level helpers the extracted solve/is_solution method
        # bodies may reference (e.g. `_is_empty`, `_single_thread_blas`,
        # plus the try/except that defines `threadpool_limits`).
        helpers_block = ""
        if class_info.get('module_level_helpers'):
            helpers_block = "\n\n" + "\n\n".join(
                class_info['module_level_helpers']
            )
...
{return_code}"""
    ...
    def create_task_files(self, task_name: str, output_dir: Optional[str] = None) -> str:
        ...
        # Upstream OpenEvolve evaluator under an unambiguous name so the
        # SMCEvolve wrapper can import from it.
        with open(output_dir / "openevolve_evaluator.py", "w") as f:
            f.write(evaluator)

        # OpenEvolve config (kept for reference; SMCEvolve uses a Hydra
        # config at configs/problem/algotune_<name>.yaml instead).
        with open(output_dir / "config.yaml", "w") as f:
            f.write(config)

        # SMCEvolve wrapper: evaluate(program: str) -> float.
        smc_wrapper = '''"""SMCEvolve evaluator — thin wrapper around the OpenEvolve evaluator.
...
'''
        ...
        try:
            ...
            description = f"AlgoTune task: {task_name}"
        task_md = (
            f"# AlgoTune: {task_name}\n\n"
            f"{description}\n\n"
            "---\n\n"
            "Define a class `<TaskName>` with a `solve(self, problem)` method "
            "and expose a module-level `run_solver(problem)` entry point.\n\n"
            "Reward = AlgoTune speedup vs. the reference `solve` "
            "implementation (primary fitness = `speedup_score`). Invalid "
            "solutions score 0.0. See `openevolve_evaluator.py` for the "
            "exact timing harness and validity predicate.\n"
        )
