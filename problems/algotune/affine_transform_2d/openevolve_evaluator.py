"""
Evaluator for the affine_transform_2d task with baseline comparison
...
def setup_algotune_paths():
    ...
    possible_algotune_paths = [
        Path("/local/storage2/jiang.2880/AlgoTune"),
...
if setup_algotune_paths():
    try:
        ...
        # Import the specific affine_transform_2d task to register it
        from AlgoTuneTasks.affine_transform_2d.affine_transform_2d import AffineTransform2D
        print("Successfully imported AlgoTune tasks and affine_transform_2d")
...
def evaluate(program_path, config=None):
    """
    Enhanced evaluation with baseline comparison for affine_transform_2d task.
    ...
    try:
        ...
        task_class = None
        if "affine_transform_2d" in TASK_REGISTRY:
            task_class = TASK_REGISTRY["affine_transform_2d"]
            print(f"Successfully loaded affine_transform_2d task from registry")
        else:
            print(f"Error: affine_transform_2d task not found in TASK_REGISTRY")
            print(f"Available tasks: {list(TASK_REGISTRY.keys())}")
            raise Exception("Could not load affine_transform_2d task from AlgoTune registry")
            ...
            try:
                ...
                if evolved_result["results"]:
                    ...
                    try:
                        ...
                        evolved_solver = program.AffineTransform2D()
