"""
Evaluator for the lu_factorization task with baseline comparison
...
if setup_algotune_paths():
    try:
        ...
        # Import the specific lu_factorization task to register it
        from AlgoTuneTasks.lu_factorization.lu_factorization import LUFactorization
        print("Successfully imported AlgoTune tasks and lu_factorization")
...
def evaluate(program_path, config=None):
    """
    Enhanced evaluation with baseline comparison for lu_factorization task.
    ...
    try:
        ...
        task_class = None
        if "lu_factorization" in TASK_REGISTRY:
            task_class = TASK_REGISTRY["lu_factorization"]
            print(f"Successfully loaded lu_factorization task from registry")
        else:
            print(f"Error: lu_factorization task not found in TASK_REGISTRY")
            print(f"Available tasks: {list(TASK_REGISTRY.keys())}")
            raise Exception("Could not load lu_factorization task from AlgoTune registry")
            ...
            try:
                ...
                if evolved_result["results"]:
                    ...
                    try:
                        ...
                        evolved_solver = program.LUFactorization()
